 ...
 */
    ...
    if ($total_items > 0) {
        ...
        $manufacturers = func_query("SELECT $sql_tbl[manufacturers].*, CONCAT($sql_tbl[customers].lastname,', ',$sql_tbl[customers].firstname) as provider_name, IF($sql_tbl[customers].id IS NULL,'','Y') as is_provider FROM $sql_tbl[manufacturers] LEFT JOIN $sql_tbl[customers] ON $sql_tbl[manufacturers].provider=$sql_tbl[customers].id ORDER BY $sql_tbl[manufacturers].manufacturer, $sql_tbl[manufacturers].manufacturer".($objects_per_page > 0 ? " LIMIT $first_page, $objects_per_page" : ""));
